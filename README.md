...
- **迷雾记忆**：环境每步按当前占有重算视野（3×3 滤波），地盘回缩会把已探索过的王城位置重新遮蔽、从观察量彻底消失。`FastGame.agent_observation` 把"已见过的王城"永久保留在通道 1（对齐真实 generals.io 的永久揭示；本环境王城不移动）。城堡位置则始终可通过通道 2+8（`structures_in_fog` 用全图真值城市 mask）获得
