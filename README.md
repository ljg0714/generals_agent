...
- **迷雾记忆**：环境每步按当前占有重算视野（3×3 滤波），地盘回缩会把已探索的格子重新起雾。`FastGame.agent_observation` 让观察量与真实 generals.io 对齐：① 已见过的王城永久保留在通道 1（部署时在模型输入注入同样的记忆）；② 通道 8（`structures_in_fog`）不再泄漏全图城市位置——只显示**曾经见过**的城市/山（灰雾记忆），未探索的城市藏在黑雾里，策略必须靠探索发现（修复前所有城市位置从第一步起就可见）
